<template>
  ...
  <div class="block-wrapper">
    <h2>SmartLabel Nutrition Section</h2>
    <i>This transformation outputs shape needed to render the nutrition section of a SmartLabel page.</i>
    <textarea
...
watch(() => props.block, (newValue: SmartLabelNutritionUiBlock) => {
      state.specContentString = newValue.spec
      state.passAlongOtherContent = (newValue.renderData as SmartLabelNutritionRenderData).passAlong as boolean
...
function rebuildUiBlockData(shiftInstructions: object): SmartLabelNutritionUiBlock {
  return new SmartLabelNutritionUiBlock(shiftInstructions, {passAlong: state.passAlongOtherContent})
}
